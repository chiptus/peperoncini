...
const SubItemInput = ({ subItem, onChangeSubItemValue, subItems, onDelete, onChangeSubItem, subItemName }) => {
  return (
    <div>
      <SelectField
        ...
        onChange={(e, key, selectedId) => onChangeSubItem(subItem._id, selectedId)} >
        {
          subItems.map(ing => <MenuItem value={ing._id} key={ing._id} primaryText={ing.name} />)
        }
      </SelectField>
      <TextField type="number"
        ...
        value={subItem.value}
        id={'ing' + subItem._id}
        onChange={({ target: { value } }) => onChangeSubItemValue(subItem._id, value)} />
